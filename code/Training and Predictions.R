...
accuracy <- sum(diag(cm1$table)) / sum(cm1$table)
...
classifer_3 <- svm(formula = Ward_Encoded ~ 
                     #IUCR_Encoded  +
...
predited_loc = predict(classifer_3 , newdata = validation_set)
predited_loc
...
# Accuracy calculation
accuracy <- sum(diag(cm3$table)) / sum(cm3$table)
print(paste("Accuracy:", accuracy))
